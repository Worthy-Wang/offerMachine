...
- [深拷贝与浅拷贝](#深拷贝与浅拷贝)
- [枚举(enum) 和 宏(#define) 的区别？](#枚举enum-和-宏define-的区别)
- [define 和typedef 的区别？](#define-和typedef-的区别)
- [using 和typedef 的区别？](#using-和typedef-的区别)
- [内联函数（inline）是什么？与宏的区别是什么？](#内联函数inline是什么与宏的区别是什么)
...
## 堆和栈的区别？
1. **性质**：栈由编译器自动分配释放，；堆由程序员手动分配释放。
2. **空间**：栈的空间有限，通常为8M，且生长方向向下，朝着低字节方向生长；堆可以拥有较大的空间，堆的生长方向向上，朝着高字节的方向生长；
3. **效率**：栈的分配由编译器完成，堆的分配由程序员完成，所以栈的效率比堆高很多。
4. **内部结构**：栈内部结构类似于数据结构中的栈；堆内部结构类似于数据结构中的链表。
5. **碎片**：堆会产生外部碎片，栈不会；
...
**注意**：
1.	友元关系没有传递性，也不能够被继承。
2.	友元并不属于类成员（也就没有this指针），所以放在public, protected, private 均可，没有关系。
